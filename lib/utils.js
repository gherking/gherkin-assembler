...
const utils = {
     ...
     */
    getStepGroups(steps, options) {
        options = utils.config(options);
        if (!options.separateStepGroups) {
            return [];
        }
        let keywords = steps.map(step => step.keyword);
        for (let i = 0; i < keywords.length; ++i) {
            if (['And', 'But'].indexOf(keywords[i]) > -1) {
                keywords[i] = keywords[i-1];
            }
        }
        let groupStarts = [];
        keywords.forEach((keyword, i) => {
            if (i && keyword === 'When' && keywords[i-1] != keyword) {
                groupStarts.push(i);
            }
        });
        return groupStarts;
